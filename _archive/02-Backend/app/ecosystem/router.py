"""FastAPI router exposing the ecosystem surface."""
...
async def list_plugins(
    ...
    records = get_plugin_registry().all()
    if state:
...
async def install_plugin(
    ...
    try:
        # Try installing by id (bundled) — fall back to manifest-from-dict.
        try:
            ...
            data = json.loads(req.source) if req.source.startswith("{") else None
            if data is None:
                raise
        ...
        raise HTTPException(
            status_code=400,
            detail=api_error(ApiErrorCode.BAD_REQUEST, str(exc), 400),
        )
...
async def update_plugin(
    ...
    manager = get_plugin_manager()
    record = manager.update(plugin_id, new_version)
    get_ecosystem_monitor().record(
        "plugin.updated",
...
async def list_api_endpoints():
    return {"version": "v1", "endpoints": get_api_registry().list("v1")}
...
async def revoke_api_key(key_id: str, user=Depends(get_current_user)):
    ...
    get_ecosystem_monitor().record("api.key.revoked", {"key_id": key_id})
    get_audit_log().record(
        actor=user.get("email", user.get("id", "system")),
...
async def api_analytics(user=Depends(get_current_user)):
    return get_api_analytics().summary()
...
async def webhook_analytics():
    return get_webhook_manager().metrics()
...
async def verify_webhook_signature(
    ...
    import json
    body = json.dumps(request_body, default=str).encode("utf-8")
    valid = get_webhook_manager().verify_incoming(
...
async def list_integration_catalog():
    return {"count": len(get_integration_registry().list()), "integrations": get_integration_registry().list()}
...
async def connect_integration(
    ...
    connection = IntegrationConnection(
        id=f"int_{int(__import__('time').time() * 1000)}",
        provider=provider,
        owner_id=user.get("id", "anonymous"),
        label=req.label,
        status="connected",
    ...
    get_integration_store().add(connection)
    get_ecosystem_monitor().record(
...
async def list_marketplace_listings(
    ...
    return {
        ...
        "results": catalog.search(
            query=q, category=category, tag=tag, sort=sort, limit=limit
        ),
    }
...
# Helper for the install endpoint above
