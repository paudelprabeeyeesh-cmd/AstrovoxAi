class AstraVoxAIPersonality {
    ...
    loadMemory(){ const saved = localStorage.getItem('astravox_memory'); if(saved) this.memory = JSON.parse(saved); }
    saveMemory(){ localStorage.setItem('astravox_memory', JSON.stringify(this.memory.slice(-200))); }
    async process(input){
...
const aiCore = new AstraVoxAIPersonality();
