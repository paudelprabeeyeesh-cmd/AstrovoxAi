...
class MoERouter(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        expert_mask = torch.zeros_like(logits).scatter_(1, topk_indices, 1)
        return topk_probs, topk_indices, expert_mask


class MoELayer(nn.Module):
    def __init__(self, hidden_size: int, intermediate_size: int, num_experts: int, top_k: int = 2, dropout: float = 0.1):
        super().__init__()
