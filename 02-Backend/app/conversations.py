...
from .schemas import ConversationOut, MessageOut, ConversationSearchOut
...
def get_conversation(conv_id: str, user_id: str) -> ConversationOut:
    with get_db() as conn:
        row = conn.execute("SELECT id, title, created_at FROM conversations WHERE id = ? AND user_id = ?", (conv_id, user_id)).fetchone()
        if not row:
            raise ValueError("Conversation not found")
        return ConversationOut(id=row["id"], title=row["title"], created_at=datetime.fromisoformat(row["created_at"]))
...
def list_conversations(user_id: str) -> list[ConversationOut]:
    with get_db() as conn:
        ...
        return [ConversationOut(id=r["id"], title=r["title"], created_at=datetime.fromisoformat(r["created_at"])) for r in rows]
...
def search_conversations(user_id: str, query: str) -> list[ConversationSearchOut]:
    ...
    with get_db() as conn:
        ...
        return [ConversationSearchOut(id=r["id"], title=r["title"], created_at=datetime.fromisoformat(r["created_at"]), message_count=r["message_count"]) for r in rows]

def add_message(conversation_id: str, role: str, content: str) -> MessageOut:
    msg_id = str(uuid.uuid4())
    with get_db() as conn:
        conn.execute("INSERT INTO messages (id, conversation_id, role, content) VALUES (?, ?, ?, ?)", (msg_id, conversation_id, role, content))
        conn.commit()
    return MessageOut(id=msg_id, role=role, content=content, created_at=datetime.utcnow())
...
def get_messages(conversation_id: str, user_id: str) -> list[MessageOut]:
    with get_db() as conn:
        ...
        return [MessageOut(id=r["id"], role=r["role"], content=r["content"], created_at=datetime.fromisoformat(r["created_at"])) for r in rows]
