...
async def track_event(
    event: TelemetryEvent, authorization: str = Header(None)
) -> Dict[str, Any]:
...
async def track_page_view(
    event: PageViewEvent, authorization: str = Header(None)
) -> Dict[str, Any]:
            ...
            .insert(
                {
                    ...
                    "category": "navigation",
                    "metadata": json.dumps(
                        {"page": event.page, "referrer": event.referrer}
                    ),
                    "timestamp": timestamp,
                }
            )
...
async def track_error(
    event: ErrorEvent, authorization: str = Header(None)
) -> Dict[str, Any]:
...
async def track_user_action(
    event: UserActionEvent, authorization: str = Header(None)
) -> Dict[str, Any]:
...
async def get_telemetry_stats(
    authorization: str = Header(None), limit: int = 100, offset: int = 0
) -> Dict[str, Any]:
    ...
    try:
        response = (
            supabase.table("telemetry_events")
            .select("id, event_name, category, timestamp")
            .eq("user_id", user_id)
