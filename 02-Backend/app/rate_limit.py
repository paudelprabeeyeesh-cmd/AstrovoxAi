    ...
    async def dispatch(self, request: Request, call_next):
        if request.url.path not in ["/health", "/metrics", "/docs", "/openapi.json"]:
            ...
            if auth.startswith("Bearer user-"):
                ...
                with get_db() as conn:
                    count = conn.execute("SELECT COUNT(*) as c FROM usage WHERE user_id = ? AND date(created_at) = ?", (user_id, today)).fetchone()["c"]
                if count >= limits["requests"]:
                    raise HTTPException(status_code=429, detail=f"Daily limit reached for {plan} plan. Upgrade to continue.")
