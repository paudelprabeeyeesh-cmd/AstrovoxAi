...
def create_comment(post_id: str, user_id: str, content: str) -> dict:
    ...
    with get_db() as conn:
        conn.execute("INSERT INTO comments (id, post_id, user_id, content, created_at) VALUES (?, ?, ?, ?, ?)",
...
def list_comments(post_id: str) -> list[dict]:
    with get_db() as conn:
        rows = conn.execute("SELECT id, user_id, content, created_at FROM comments WHERE post_id = ? ORDER BY created_at ASC", (post_id,)).fetchall()
