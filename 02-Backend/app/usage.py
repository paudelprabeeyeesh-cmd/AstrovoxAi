...
def record_usage(user_id: str, tokens: int, cost: float, model: str, cached: bool = False, error: str = None):
    with get_db() as _conn:
        _conn.execute("CREATE TABLE IF NOT EXISTS usage (id TEXT PRIMARY KEY, user_id TEXT NOT NULL, tokens INTEGER NOT NULL, cost REAL NOT NULL, model TEXT NOT NULL, cached INTEGER DEFAULT 0, error TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
        _conn.commit()
