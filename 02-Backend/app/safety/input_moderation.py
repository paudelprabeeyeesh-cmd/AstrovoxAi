...
class ViolenceDetector:
    ...
    def detect(self, text: str) -> Optional[ModerationResult]:
        ...
        if hits:
            return ModerationResult(safe=False, categories=["violence"], confidence=0.9, flags=hits)
        return None
...
class SelfHarmDetector:
    ...
    def detect(self, text: str) -> Optional[ModerationResult]:
        ...
        hits = [kw for kw in self.KEYWORDS if kw in lowered]
        if hits:
            return ModerationResult(safe=False, categories=["self_harm"], confidence=0.95, flags=hits)
        return None
...
class InputModerator:
    def __init__(self):
        ...
        self._pipeline = [self.toxicity.classify, self.violence.detect, self.self_harm.detect]
