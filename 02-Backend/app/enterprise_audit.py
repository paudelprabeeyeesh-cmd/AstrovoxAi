...
def list_audit_logs(user_id: str, limit: int = 100) -> list[dict]:
    with get_db() as conn:
        rows = conn.execute("SELECT id, action, metadata, created_at FROM enterprise_audit_logs WHERE user_id = ? ORDER BY created_at DESC LIMIT ?", (user_id, limit)).fetchall()
        return [{"id": r["id"], "action": r["action"], "metadata": r["metadata"], "created_at": r["created_at"]} for r in rows]
