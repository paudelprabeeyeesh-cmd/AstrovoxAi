...
def get_memory(memory_id: str) -> MemoryOut:
    with get_db() as conn:
        row = conn.execute("SELECT id, key, value, created_at FROM memories WHERE id = ?", (memory_id,)).fetchone()
        if not row:
            raise ValueError("Memory not found")
        return MemoryOut(id=row["id"], key=row["key"], value=row["value"], created_at=datetime.fromisoformat(row["created_at"]))

def list_memories(user_id: str) -> list[MemoryOut]:
    with get_db() as conn:
        rows = conn.execute("SELECT id, key, value, created_at FROM memories WHERE user_id = ? ORDER BY created_at DESC", (user_id,)).fetchall()
        return [MemoryOut(id=r["id"], key=r["key"], value=r["value"], created_at=datetime.fromisoformat(r["created_at"])) for r in rows]

def update_memory(memory_id: str, user_id: str, data: MemoryUpdate) -> MemoryOut:
    with get_db() as conn:
        conn.execute("UPDATE memories SET value = ? WHERE id = ? AND user_id = ?", (data.value, memory_id, user_id))
        conn.commit()
    return get_memory(memory_id)
...
def export_memories(user_id: str) -> dict:
    ...
    return {"user_id": user_id, "exported_at": datetime.utcnow().isoformat(), "memories": [m.dict() for m in memories]}
