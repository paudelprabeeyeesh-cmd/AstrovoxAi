...
def get_sla(account_id: str) -> dict:
    with get_db() as conn:
        row = conn.execute("SELECT id, tier, uptime_guarantee, response_time_hours, created_at FROM slas WHERE account_id = ?", (account_id,)).fetchone()
        if not row:
            raise ValueError("SLA not found")
        return {"id": row["id"], "tier": row["tier"], "uptime_guarantee": row["uptime_guarantee"], "response_time_hours": row["response_time_hours"], "created_at": row["created_at"]}
