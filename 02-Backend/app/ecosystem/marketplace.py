"""Integration marketplace: catalog, search, ratings, installation flows."""
...
from .plugins import PluginManifest, PluginState, get_plugin_manager
...
class MarketplaceCatalog:
    ...
    def add_rating(self, listing_id: str, rating: ListingRating) -> Optional[Listing]:
        ...
        listing.rating_avg = sum(r.stars for r in ratings) / listing.rating_count
        return listing
    ...
    def install(
        ...
        record = manager.install(listing_id, permissions=permissions, config=config)
    ...
    def toggle(
        self, listing_id: str, enabled: bool
    ) -> Dict[str, Any]:
        manager = get_plugin_manager()
...
def seed_default_catalog() -> None:
    ...
    for plugin_id in bundled_ids:
        try:
            ...
            logger.debug("Skipping bundled plugin %s: %s", plugin_id, exc)
    # Lightly feature the catalog
