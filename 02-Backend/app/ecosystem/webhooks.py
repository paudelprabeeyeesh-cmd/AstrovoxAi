"""Webhook platform: incoming + outgoing webhooks with retries and DLQ."""
...
class WebhookSubscription:
    ...
    rate_limit: int = 120  # deliveries per minute
...
class WebhookDeliveryStore:
    """Append-only delivery log backed by a JSON file."""
...
def dispatch_event(event: str, payload: Dict[str, Any], owner_id: Optional[str] = None) -> List[Dict[str, Any]]:
    """Synchronous dispatcher used from places we cannot await from.

    Falls back to scheduling the deliveries on the running event loop if one
    exists; otherwise the delivery is queued for the next call to ``drain``.
    """
