"""Public developer API platform for AstrovoxAI.

Includes:
- REST surface definitions for the external developer API
- API key management with hashed secrets
- OAuth2-style authorization code flow (for first-party integrations)
- Rate limit definitions and analytics aggregation
- Standardized error envelope
- Webhook signature helpers (HMAC SHA-256)
- Versioned namespace registration
"""
...
def api_error(code: ApiErrorCode, message: str, status: int, **details: Any) -> Dict[str, Any]:
    return ApiError(code, message, status, details or None).to_dict()
...
class TokenBucket:
    ...
    def consume(self, amount: int = 1) -> Tuple[bool, int]:
        ...
        if len(self._hits) + amount > self.limit:
            retry = int(self.window - (now - self._hits[0]))
...
class RateLimiter:
    """Coordinates multiple buckets keyed by scope."""
...
class ApiKeyStore:
    ...
    A production deployment should back this with a database; the public
    methods are deliberately small so they can be implemented atop any
    persistent store.
    """
...
class OAuthServer:
    """Lightweight OAuth 2.0 server supporting the flows we expose."""
    ...
    def exchange_code(self, code: str, client_id: str, redirect_uri: str) -> Optional[OAuthToken]:
...
async def require_api_key(
    ...
    store = get_key_store()
    if x_api_key:
        record = store.verify(x_api_key, x_api_key)
        if record is None:
            raise HTTPException(status_code=401, detail=api_error(ApiErrorCode.UNAUTHENTICATED, "Invalid API key", 401))
        return record
    if authorization and authorization.lower().startswith("bearer "):
        token = authorization.split(" ", 1)[1].strip()
        record = store.verify(token, token)
        if record is not None:
            return record
    raise HTTPException(status_code=401, detail=api_error(ApiErrorCode.UNAUTHENTICATED, "Missing credentials", 401))


async def require_scope(scope: str, key: ApiKey = Depends(require_api_key)) -> ApiKey:
    if scope not in key.scopes:
        raise HTTPException(
            status_code=403,
            detail=api_error(ApiErrorCode.FORBIDDEN, f"Missing scope: {scope}", 403),
        )
    return key
...
async def track_request(
    ...
    """Decorator-style helper used by middleware to record API metrics."""

    start = time.time()

    def _finalize(status: int, error_code: Optional[str] = None) -> None:
        latency = (time.time() - start) * 1000
        try:
            get_api_analytics().record(
                ApiCall(
                    timestamp=start,
    ...
    request.state.astrovox_finalize = _finalize  # type: ignore[attr-defined]
    return response_factory()


def build_error_response(code: ApiErrorCode, message: str, status: int, **details: Any) -> HTTPException:
    return HTTPException(status_code=status, detail=api_error(code, message, status, **details))