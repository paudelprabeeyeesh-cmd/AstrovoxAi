...
Each integration provides OAuth-friendly helpers, typed request/response
schemas, and a normalized status surface.  These are exposed as
both direct Python APIs and as FastAPI endpoints so users can build
workflows across platforms without writing bespoke HTTP code.
"""
...
class IntegrationStore:
    """In-memory store for integration connections."""
...
class OAuthFlow:
    ...
    def authorization_url(self, state: str, code_challenge: Optional[str] = None) -> str:
        params = {
            "response_type": "code",
        ...
        params.update(self.extra.get("authorize_params", {}))
...
# ---------------------------------------------------------------------------
# Provider actions - typed adapters
# ---------------------------------------------------------------------------


class IntegrationClient:
    """High level client that performs actions across providers.

    In production this layer wraps ``httpx`` and the provider OAuth flows.
    For unit testing and offline development it returns structured
    payloads that are obviously non-functional.
    """
