...
def create_ab_test(name: str, variants: list[str], traffic_split: list[float]) -> str:
    ...
    with get_db() as conn:
        conn.execute("INSERT INTO ab_tests (id, name, variants, traffic_split, active) VALUES (?, ?, ?, ?, ?)",
...
def get_variant(test_id: str, user_id: str) -> str:
    with get_db() as conn:
        row = conn.execute("SELECT variants, traffic_split FROM ab_tests WHERE id = ? AND active = 1", (test_id,)).fetchone()
        if not row:
            return ""
...
def record_result(test_id: str, variant: str, metric: str, value: float):
    with get_db() as conn:
        conn.execute("INSERT INTO ab_results (id, test_id, variant, metric, value, created_at) VALUES (?, ?, ?, ?, ?, ?)",
                     (str(uuid.uuid4()), test_id, variant, metric, value, datetime.utcnow().isoformat()))
        conn.commit()
