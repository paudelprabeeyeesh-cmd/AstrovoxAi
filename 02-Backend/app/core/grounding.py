...
def ground_answer(answer: str, contexts: list[dict], query: str) -> tuple[str, bool, float]:
    ...
    if not has_supporting_context(answer, contexts):
        ...
        return "I don't know. There is no information in my knowledge base to answer this question.", True, confidence
    ...
    citations = [c.get("id", "") for c in contexts[:3] if c.get("score", 0) > MAX_CITATION_DISTANCE]
    if citations:
