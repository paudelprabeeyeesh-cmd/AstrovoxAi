...
class SemanticCache:
    ...
    def _make_key(self, user_id: str, prompt_hash: str, model: str, shared: bool = False) -> str:
        user_part = "shared" if shared else user_id
        key_data = f"{user_part}:{prompt_hash}:{model}"
        return hashlib.sha256(key_data.encode()).hexdigest()
    
    def get(self, user_id: str, prompt_hash: str, model: str, shared: bool = False) -> Optional[dict]:
        if not self.redis:
            return None
    ...
    def set(self, user_id: str, prompt_hash: str, model: str, value: dict, ttl: int = 3600, shared: bool = False):
        if not self.redis:
            return
