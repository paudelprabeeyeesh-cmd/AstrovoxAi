...
def create_refresh_token(user_id: str) -> str:
    ...
    with get_db() as conn:
        conn.execute("INSERT INTO refresh_tokens (id, user_id, token_hash, expires_at) VALUES (?, ?, ?, ?)",
...
def login_user(email: str, password: str) -> dict:
    ...
    return {"access_token": access_token, "refresh_token": refresh_token, "user_id": row["id"], "email": row["email"]}
...
def refresh_access_token(refresh_token: str) -> dict:
    ...
    with get_db() as conn:
        row = conn.execute("SELECT user_id FROM refresh_tokens WHERE token_hash = ? AND expires_at > ?",
                           (token_hash, datetime.utcnow().isoformat())).fetchone()
        if not row:
            raise HTTPException(status_code=401, detail="Refresh token expired or revoked")
        user = conn.execute("SELECT id, email FROM users WHERE id = ?", (row["user_id"],)).fetchone()
        if not user:
