...
def create_ama(title: str, description: str, scheduled_at: str) -> dict:
    ...
    with get_db() as conn:
        conn.execute(
            "INSERT INTO amas (id, title, description, scheduled_at, created_at) VALUES (?, ?, ?, ?, ?)",
...
def list_amas() -> list[dict]:
    with get_db() as conn:
        rows = conn.execute(
            "SELECT id, title, description, scheduled_at, created_at FROM amas ORDER BY scheduled_at DESC"
        ).fetchall()
        return [
            {
                "id": r["id"],
