"""PagedAttention KV cache with block-based memory management."""
...
class BlockManager:
    def __init__(self, num_blocks: int, num_heads: int, head_dim: int, block_size: int, device: torch.device, dtype: torch.dtype = torch.float16):
    ...
    def allocate(self, layer_idx: int, is_prefix: bool = False, prefix_hash: Optional[str] = None) -> CacheBlock:
        if self._free_list:
            ...
            return block
        if len(self._blocks) >= self.num_blocks:
            self._evict()
        ...
        block = CacheBlock(block_id=block_id, layer_idx=layer_idx, k=k, v=v, is_prefix=is_prefix, prefix_hash=prefix_hash)
    ...
    def _evict(self) -> None:
        ...
        self._evictions += 1
        logger.debug("Evicted block %d from layer %d", victim.block_id, victim.layer_idx)
...
class PagedKVCache:
    def __init__(self, num_layers: int, num_heads: int, head_dim: int, max_seq_len: int, block_size: int, max_blocks: int, device: torch.device, dtype: torch.dtype = torch.float16):
        ...
        for layer_idx in range(num_layers):
            self._managers.append(BlockManager(max_blocks, num_heads, head_dim, block_size, device, dtype))
        self._block_tables: Dict[int, List[int]] = {}
        self._seq_lens: Dict[int, int] = {}
        self._prefix_blocks: Dict[str, List[CacheBlock]] = {}

    def reserve(self, seq_id: int, num_blocks: int, layer_idx: int) -> List[CacheBlock]:
        blocks = []
        for _ in range(num_blocks):
            block = self._managers[layer_idx].allocate(layer_idx)
            blocks.append(block)
        return blocks

    def assign_prefix(self, prefix_hash: str, blocks: List[CacheBlock]) -> None:
        for block in blocks:
            block.is_prefix = True
            block.prefix_hash = prefix_hash
        self._prefix_blocks[prefix_hash] = blocks

    def get_prefix_blocks(self, prefix_hash: str, layer_idx: int) -> Optional[List[CacheBlock]]:
    ...
    def update(self, seq_id: int, layer_idx: int, k: torch.Tensor, v: torch.Tensor, offset: int) -> None:
        ...
        if not table:
            num_blocks = (self._seq_lens.get(seq_id, k.shape[2]) + self.block_size - 1) // self.block_size
            for _ in range(num_blocks):
                table.append(self._managers[layer_idx].allocate(layer_idx).block_id)
        for i, block_id in enumerate(table):
            block = self._managers[layer_idx]._blocks.get(block_id)
            if block is None:
                continue
    ...
    def release(self, seq_id: int) -> None:
        ...
        for block_id in table:
            for layer_idx in range(self.num_layers):
                self._managers[layer_idx].free(block_id)

    def metrics(self) -> Dict[str, Any]:
        return {
            "sequences": len(self._block_tables),
            "prefixes": len(self._prefix_blocks),
            "layers": [m.metrics() for m in self._managers],
        }
...
class PagedAttention(nn.Module):
    ...
    def _get_cached_kv(self, seq_id: Optional[int], layer_idx: int, current_len: int) -> Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]:
        ...
        k_blocks = []
        v_blocks = []
        manager = self.kv_cache._managers[layer_idx]
        for block_id in table:
            block = manager._blocks.get(block_id)
