...
async def create_user_profile(user_id: str, username: str, full_name: str = None, avatar_url: str = None):
    """Create user profile"""
    try:
...
async def get_conversations(user_id: str, limit: int = 50, offset: int = 0):
    ...
    try:
        response = supabase.table("conversations").select("*").eq("user_id", user_id).eq("is_deleted", False).order("updated_at", desc=True).range(offset, offset + limit - 1).execute()
...
async def update_conversation(conversation_id: int, **kwargs):
    """Update conversation"""
    try:
        response = supabase.table("conversations").update(kwargs).eq("id", conversation_id).execute()
        return response.data[0] if response.data else None
    except Exception as e:
...
async def delete_conversation(conversation_id: int):
    ...
    try:
        response = supabase.table("conversations").update({"is_deleted": True}).eq("id", conversation_id).execute()
        return True
    except Exception as e:
...
# Message operations
async def create_message(conversation_id: int, user_id: str, role: str, content: str, model_used: str = None, tokens_used: int = None):
...
async def get_messages(conversation_id: int, limit: int = 100, offset: int = 0):
    ...
    try:
        response = supabase.table("messages").select("*").eq("conversation_id", conversation_id).order("created_at", desc=False).range(offset, offset + limit - 1).execute()
        return response.data
    except Exception as e:
...
async def get_user_memory(user_id: str, limit: int = 50):
    ...
    try:
        response = supabase.table("ai_memory").select("*").eq("user_id", user_id).order("importance", desc=True).order("created_at", desc=True).limit(limit).execute()
        return response.data
    except Exception as e:
...
async def update_user_settings(user_id: str, **kwargs):
    ...
    try:
        response = supabase.table("user_settings").update(kwargs).eq("user_id", user_id).execute()
        return response.data[0] if response.data else None
    except Exception as e:
