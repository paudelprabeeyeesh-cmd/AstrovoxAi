"""Tensor parallelism: shard linear layers, all-reduce across ranks."""
...
class TensorParallelConfig:
    ...
    rank: int = 0
    shard_dim: int = 0
    all_reduce_dtype: torch.dtype = torch.float32
...
class TensorParallelism:
    ...
    def shard_linear(self, linear: nn.Linear, dim: int = 0) -> nn.Linear:
        ...
        if dim == 0:
            ...
            bias_shard = linear.bias.data[start:end].clone() if has_bias else None
            sharded = nn.Linear(in_features, shard_out, bias=has_bias)
    ...
    def apply_to_model(self, model: nn.Module, dim: int = 0) -> nn.Module:
        for name, module in list(model.named_modules()):
            if isinstance(module, nn.Linear):
                parent_name, _, attr = name.rpartition(".")
                parent = model if parent_name == "" else self._get_attr(model, parent_name)
                sharded = self.shard_linear(module, dim=dim)
                if parent is not None:
                    setattr(parent, attr, sharded)
        return model

    @staticmethod
    def _get_attr(model: nn.Module, name: str) -> Optional[nn.Module]:
        parts = name.split(".")
        obj = model
        for part in parts:
