...
class ExpertParallelismWithAlltoAll:
    ...
    def alltoall(self, tokens: torch.Tensor, expert_indices: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        send_buffer = tokens.clone()
        return send_buffer, torch.zeros(batch_size, device=tokens.device)
    ...
    def forward(self, hidden_states: torch.Tensor, gate_logits: torch.Tensor, top_k: int = 2) -> torch.Tensor:
        ...
        for device_id, (tokens, probs) in dispatched.items():
            expert_ids = list(dict.fromkeys(top_k_indices.view(-1)[:len(tokens)].tolist()))
            expert_outputs = []
            for idx, expert_id in enumerate(expert_ids[:len(tokens)]):
