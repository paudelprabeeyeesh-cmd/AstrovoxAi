...
class SpeakerIdentifier:
    ...
    def enroll_speaker(self, speaker_id: str, audio_paths: List[str], metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        for path in audio_paths:
            if os.path.exists(path):
                data, _ = AudioUtils.load_audio(path)
                embedding = self.feature_extractor.extract(data)
    ...
    def identify_speaker(self, audio_path: str, threshold: float = 0.7) -> Dict[str, Any]:
        ...
        query_data, _ = AudioUtils.load_audio(audio_path)
        query_embedding = self.feature_extractor.extract(query_data)
        best_match = None
        best_score = -1.0
        ...
        return {"speaker_id": None, "confidence": round(best_score, 4), "status": "unknown", "threshold": threshold}
