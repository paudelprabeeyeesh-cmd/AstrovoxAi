...
Unified memory system for the AI core with:
- Short-term memory
- Long-term memory
- Semantic memory
- Episodic memory
...
class MemoryScorer:
    def score(
        ...
        if memory_type in (MemoryType.USER_PREFERENCE, MemoryType.SEMANTIC):
            score += 0.2
        content_lower = content.lower()
        high = ["important", "critical", "essential", "must", "required", "remember"]
        low = ["just", "only", "maybe", "perhaps", "might", "temporary"]
        score += sum(0.1 for kw in high if kw in content_lower)
        score -= sum(0.1 for kw in low if kw in content_lower)
    ...
    def detect_conflicts(self, memories: List[MemoryFragment]) -> List[Tuple[MemoryFragment, MemoryFragment, str]]:
        ...
        for i in range(len(memories)):
            for j in range(i + 1, len(memories)):
                ...
                if a.user_id != b.user_id or a.memory_type != b.memory_type:
                    continue
...
class MemoryBrain:
    """Unified memory brain for ASTROVOX AI core."""
    ...
    def remember(
        ...
        score = self._scorer.score(content, memory_type, user_explicit=user_explicit, confidence=confidence)
    ...
    def get_user_preferences(self, user_id: str) -> List[MemoryFragment]:
        return self.recall(user_id, memory_type=MemoryType.USER_PREFERENCE)
    ...
    def detect_conflicts(self, user_id: str) -> List[Tuple[MemoryFragment, MemoryFragment, str]]:
        memories = [
            ...
            if mid in self._memories and not self._memories[mid].deleted
        ]
    ...
    def get_stats(self, user_id: str) -> Dict[str, Any]:
        ...
        return {
            "total": len(memories),
            "by_type": {t.value: sum(1 for m in memories if m.memory_type == t) for t in MemoryType},
