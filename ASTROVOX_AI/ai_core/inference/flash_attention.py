"""FlashAttention v2/v3 memory-efficient attention for AI core."""
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if self.config.use_cuda_kernel:
            try:
                ...
                out = flash_attn_func(q, k, v, dropout_p=self.dropout.p, causal=self.causal)
                return self.out_proj(out.view(B, T, C))
            except ImportError:
                logger.warning("flash_attn not installed, falling back to memory-efficient SDPA")
        ...
        for bi in range(B):
            for hi in range(H):
                ...
                for j in range(num_blocks):
                    k_j = k_block[j]
                    v_j = v_block[j]
                    s_ij = torch.matmul(q_block, k_j.transpose(-2, -1)) * self.scale
                    if self.causal:
