"""Emoji handler for ASTROVOX_AI tokenization."""
...
class EmojiTokenizer:
    def __init__(self, config: Optional[EmojiTokenizerConfig] = None):
        ...
        self._load_vocab()
        logger.info("EmojiTokenizer initialized")
    ...
    def describe(self, text: str) -> List[Dict[str, str]]:
        ...
        for match in self.emoji_pattern.finditer(text):
            ...
            results.append({"emoji": emoji, "start": match.start(), "end": match.end(), "name": name})
        return results


class EmojiHandler:
    def __init__(self, replace: bool = False, placeholder: str = "<emoji>"):
        self.tokenizer = EmojiTokenizer(EmojiTokenizerConfig(replace_with_placeholder=replace, placeholder=placeholder))

    def process(self, text: str) -> str:
        return self.tokenizer.replace_emojis(text)

    def extract(self, text: str) -> List[str]:
        return self.tokenizer.extract_emojis(text)
