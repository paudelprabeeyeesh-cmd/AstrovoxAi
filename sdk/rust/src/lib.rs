...
impl AstrovoxClient {
    ...
    async fn request(&self, method: &str, path: &str, body: Option<Value>) -> Result<Value, reqwest::Error> {
    ...
    pub async fn create_webhook(&self, url: &str, events: Vec<&str>) -> Result<Value, reqwest::Error> {
        let body = serde_json::json!({
            "url": url,
            "events": events
        });
        self.request("POST", "/webhooks", Some(body)).await
    }
